...
// Ported from OrthonormalFrame
// Richard S. Wright Jr.
// Code by Richard S. Wright Jr.

#include <math3d.h>	// Need to link to this too...
#include <gl/gl.h>
...
class GLFrame
    ...
    public:
        ...
        inline void SetForwardVector(const M3DVector3f vDirection) {
			m3dCopyVector3f(vForward, vDirection); }

        inline void SetForwardVector(float x, float y, float z)
            { vForward[0] = x; vForward[1] = y; vForward[2] = z; }

        inline void GetForwardVector(M3DVector3f vVector) { m3dCopyVector3f(vVector, vForward); }

        /////////////////////////////////////////////////////////////
        // Set Up Direction
        inline void SetUpVector(const M3DVector3f vDirection) {
			m3dCopyVector3f(vUp, vDirection); }

        inline void SetUpVector(float x, float y, float z)
			{ vUp[0] = x; vUp[1] = y; vUp[2] = z; }

        inline void GetUpVector(M3DVector3f vVector) { m3dCopyVector3f(vVector, vUp); }
		...
		inline void GetXAxis(M3DVector3f vVector) { m3dCrossProductf(vVector, vUp, vForward); }
		...
		void GetMatrix(M3DMatrix44f	matrix, bool bRotationOnly = false)
			...
			M3DVector3f vXAxis;
			m3dCrossProductf(vXAxis, vUp, vForward);
            ...
            // Y Column
			m3dSetMatrixColumn44f(matrix, vUp, 1);
            matrix[7] = 0.0f;       
                                    
            // Z Column
			m3dSetMatrixColumn44f(matrix, vForward, 2);
            matrix[11] = 0.0f;
			...
			if(bRotationOnly == true)
				...
				m3dSetMatrixColumn44f(matrix, vOrigin, 3);
		...
		// Transform frame, but billboard towards camera. This is much simpler than
		// many books make it sound. All you have to do is take the rotational part of
		// the camera matrix, and use this frames translation values.
		// Linear algebra and vectors are your friends. Trigonometry is computational
		// black hole.
		// You get true spherical billboarding for almost nothing computationaly.
		void ApplyBillboardTransform(GLFrame& camera)
			{
			// Get camera orientation
			M3DVector3f cameraX, cameraForward, cameraUp;
			camera.GetForwardVector(cameraForward);
			camera.GetUpVector(cameraUp);

			// Must calculate the camera's X vector... just a cross product
			m3dCrossProductf(cameraX, cameraUp, cameraForward);

			// OK, build a rotation matrix out of our components
			M3DMatrix44f rotMat;

			// X Column
            m3dSetMatrixColumn44f(rotMat, cameraX, 0);
            rotMat[3] = 0.0f;
           
            // Y Column
			m3dSetMatrixColumn44f(rotMat, cameraUp, 1);
            rotMat[7] = 0.0f;       
                                    
            // Z Column
			m3dSetMatrixColumn44f(rotMat, cameraForward, 2);
            rotMat[11] = 0.0f;

            // Translation column comes from the billboards location.
			m3dSetMatrixColumn44f(rotMat, vOrigin, 3);
            rotMat[15] = 1.0f;

			// Apply the whole schebang!
			glMultMatrixf(rotMat);
			}
        ...
        void RotateLocalX(float fAngle)
			...
			m3dCopyVector3f(vForward, newVect);
		...
		void Normalize(void)
			...
			m3dCrossProductf(vForward, vCross, vUp);	

			// Also check for unit length...
			m3dNormalizeVectorf(vUp);
			m3dNormalizeVectorf(vForward);
			}
